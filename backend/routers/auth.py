...
import models
from auth_utils import generate_login, generate_password, hash_password, verify_password, create_access_token, decode_token
...
def telegram_register(req: TelegramAuthRequest, db: Session = Depends(get_db)):
    ...
    if existing:
        return {"already_exists": True, "login": existing.login}
    ...
    db.commit()
    db.refresh(user)
    return {"already_exists": False, "login": login, "password": password, "user_id": user.id}
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "user": {
            ...
            "balance": user.balance, "telegram_id": user.telegram_id,
